...
import { BIOLOGY_SIMULATIONS, Simulation } from "../dataBio";
import { MdOutlineBiotech } from "react-icons/md";
import { IoArrowBack } from "react-icons/io5";
import SimulationCard from "@/app/components/card/SimulationCard";

export default function PhETBiologyPage() {
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <div className="p-2 rounded-lg bg-orange-100">
                <MdOutlineBiotech className="text-orange-600 text-2xl" />
              </div>
              <span className="text-sm font-bold text-orange-600 tracking-wider uppercase">
                PhET Simulations
              </span>
            </div>
            <h1 className="text-3xl font-extrabold text-slate-900">Biology</h1>
          </div>
        </div>

        {/* Grid Layout */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {BIOLOGY_SIMULATIONS.map((sim) => (
            <SimulationCard
              key={sim.id}
              simulation={sim}
