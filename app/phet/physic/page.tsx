...
import { PHYSICS_SIMULATIONS, Simulation } from "../dataPhysic";
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <div className="p-2 rounded-lg bg-orange-100">
                <TbBolt className="text-orange-600 text-2xl" />
              </div>
              <span className="text-sm font-bold text-orange-600 tracking-wider uppercase">
                PhET Simulations
              </span>
            </div>
            <h1 className="text-3xl font-extrabold text-slate-900">Physics</h1>
          </div>
        </div>

        {/* Grid Layout */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {PHYSICS_SIMULATIONS.map((sim) => (
            <SimulationCard
              key={sim.id}
              simulation={sim}
